...
def show_character_differences(input_file_byte_counts, target_1_byte_counts,target_2_byte_counts):
    ...
    for byte in sorted(bytes):
        ...
        target_1_count = target_1_byte_counts.get(byte, 0)
        ...
        if 32 <= byte <= 126: #actual printable ascii range
            ...
            ascii_repr = f"0x{byte:02x}"  # if no printable ascii show hex value

        logging.info(
            f"Byte {byte} ({ascii_repr}): "
            f"in_file_count={input_file_byte_count}, t1_count={target_1_count}, t2_count={target_2_count}, t_count={total_target_bytes} "
        )
